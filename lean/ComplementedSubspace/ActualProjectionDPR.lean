...
/-! Primal DPR obstructions for the actual existential alternating projector. -/
...
theorem actualProjection_range_chiDPR_eq_top : chiDPR T.range = ⊤ := by
  ...
  have h := recursiveDiagonalRange_chiDPR_eq_top s s.alternatingBlockProjection
...
theorem actualProjection_complement_range_chiDPR_eq_top :
    chiDPR (ContinuousLinearMap.id ℝ (Ambient s.toBlockParameters) - T).range = ⊤ := by
  ...
  have h := recursiveDiagonalRange_chiDPR_eq_top s Q (norm_nonneg Tc) hbound hIdem
