...
/-! Infinite DPR for the continuous dual of either recursively constructed range. -/
...
theorem recursiveDiagonalRange_dual_chiDPR_eq_top {η : ℝ}
    ...
    chiDPR (StrongDual ℝ (lpDiagonal 2 P hC hP).range) = ⊤ := by
  ...
  have hDK : 16 * K₀ ≤ D := by
    ...
    linarith
  have hlower := @frame_product_le_chiDPR_dual
