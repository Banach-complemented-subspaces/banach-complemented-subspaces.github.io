...
/-! Dual DPR obstructions for the actual existential alternating projector. -/
...
theorem actualProjection_range_dual_chiDPR_eq_top : chiDPR (StrongDual ℝ T.range) = ⊤ := by
  ...
  have h := recursiveDiagonalRange_dual_chiDPR_eq_top s s.alternatingBlockProjection
...
theorem actualProjection_complement_range_dual_chiDPR_eq_top :
    chiDPR (StrongDual ℝ (ContinuousLinearMap.id ℝ (Ambient s.toBlockParameters) - T).range) = ⊤ := by
  ...
  have h := recursiveDiagonalRange_dual_chiDPR_eq_top s Q (norm_nonneg Tc) hbound hIdem
